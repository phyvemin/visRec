...
def create_depth_map(pil_image):
    """
    Creates a 3-channel depth map for the input PIL image.
    
    Args:
        pil_image (PIL.Image): Input PIL image.
    
    Returns:
        np.ndarray: 3-channel depth map of the image.
    """
    ...
    for inputs, gt_image, img_name, prompt in test_loader:
        ...
        with model.ema_scope():
            ...
            for index in range(0, len(inputs)):
                depth = create_depth_map(gt_image[index])
                print("depth first",depth.shape)
                cur_gt_img = rearrange(gt_image[index], 'h w c -> c h w')
                gt_img.append(torch.clamp((cur_gt_img + 1.0) / 2.0, min=0.0, max=1.0))
                depths.append(torch.clamp((depth + 1.0) / 2.0, min=0.0, max=1.0))
            ...
            for i in range(0, len(inputs)):
                ...
                print('depth here',depth.shape)
                depth = depth.unsqueeze(0)
                print("depth last",depth.shape)
                    ...
                    for copy_idx, img_t in enumerate(samples_t):
                        ...
                        if (copy_idx == 0):
                            Image.fromarray(img_t).save(
                                f"{output_path}/depth-picture-gene/batch_" + str(batch_pos) + "_test_" + str(index) + "_" + img_name[
                                    index] + "_gt.png")
                            Image.fromarray(img_t).save(
                                f"{output_path}/depth-picture-gene-onlygt/batch_" + str(batch_pos) + "_test_" + str(index) + "_" + img_name[
                                    index] + "_gt.png")
                ...
                for j in range(0, num_samples):
                    generated_image[j].save(f"{output_path}/depth-picture-gene/batch_" + str(batch_pos) + "_test_" + str(index) + "_" + img_name[
                            index] + "_" + str(j+1) + ".png")
