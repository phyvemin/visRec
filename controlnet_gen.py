...
import sys
sys.path.append("/DATA/MTP_b21260/BrainVis/ControlNet")  # Adjust if needed
import warnings
...
test_img_names_file=datapath+'test_image_names6.pth'
test_seq_file=datapath+'test_seqs6.pth'
dff_model_path = "pretrained_model/v1-5-pruned-emaonly.ckpt"
dff_yaml_path = "pretrained_model/config15.yaml"
test_pred_file=datapath+'test_pred6.pth'
output_path="picture"
...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        depth_transform_test = transforms.Compose([
            transforms.ToTensor(),
            transforms.Resize((512, 512))
        ])
        gt_image = np.array(image) / 255.0
        gt_image = img_transform_test(gt_image)
        map = Image.open(f'/DATA/MTP_b21260/Depthmapping/depthmaps/{img_label}/{self.image_names[idx] + img_file_type}').convert('RGB')
        detected_map = np.array(map) / 255.0
        print(f'/DATA/MTP_b21260/Depthmapping/depthmaps/{img_label}/{self.image_names[idx] + img_file_type}')
        # print(detected_map.shape)
        # detected_map = torch.tensor(detected_map)
        detected_map = depth_transform_test(detected_map).to(torch.float32)

        prompt = propmt_dict[lable_number_dict[str(self.labels[idx])]]

        return input_vec, gt_image, self.image_names[idx], prompt, detected_map
...
model.load_state_dict(load_state_dict('./ControlNet/models/control_sd15_depth.pth', location='cuda'), strict=False)
model = model.to(device1)
ddim_sampler = DDIMSampler(model)
model.control_scales = [1.0 * (0.825 ** float(12 - i)) for i in range(13)] if False else ([1.0] * 13)  # Magic number. IDK why. Perhaps because 0.825**12<0.01 but 0.826**12>0.01
shape = (4, 512 // 8, 512 // 8)
'''
...
with torch.no_grad():
    for inputs, gt_image, img_name, prompt, detected_map in test_loader:
        output_path = "picture"
        gt_img = []
        latent = []
        images_list = []
        ...
        control = torch.stack([detected_map for _ in range(num_samples)], dim=0)
        control = control.squeeze(1)
        control = control.to(torch.float32).to(device1)
        # print(control.shape)
        ...
        with model.ema_scope():
            ...
            for index in range(0, len(inputs)):
                ...
                latent.append(clip_encoded[index].reshape(77, 768).to(torch.float32).to(device1))
            ...
            for i in range(0, len(inputs)):
                ...
                print(f"control shape: {control.shape}")  # Should be (batch_size, channels, height, width)
                print(f"c_crossattn shape: {cond['c_crossattn'][0].shape}")  # Should be (batch_size, 77, 768)

                print(f"rendering {num_samples} examples in {ddim_steps} steps.")

                samples_ddim, _ = ddim_sampler.sample(S=ddim_steps,
                                                 conditioning=cond,
                ...
                if output_path is not None:
                    ...
                    for copy_idx, img_t in enumerate(samples_stage2):
                        ...
                        if (copy_idx == 0):
                            Image.fromarray(img_t).save(
                                "inter-imgs/batch_" + str(batch_pos) + "_test_" + str(index) + "_" + img_name[
                                    index] + "_inter.png")
                ...
                for j in range(0, num_samples):
                    # Convert ground truth image from tensor to numpy (H, W, C)
                    sample_t = (255. * torch.cat([gt_img[i].unsqueeze(dim=0).detach().cpu(), x_samples_ddim.detach().cpu()],
                                             dim=0).numpy()).astype(np.uint8)[0]
                    gt_np = rearrange(sample_t, 'c h w -> h w c')
                    # gt_np = (gt_image[i] * 255).byte().cpu().numpy()  # (H, W, C)

                    # Convert detected map to (H, W, C) with 3 channels
                    detected_map_np = (detected_map[0].permute(1, 2, 0) * 255).byte().cpu().numpy()  # (C, H, W) or (H, W)

                    # Convert generated image to numpy
                    generated_np = np.array(generated_image[j])  # Already (H, W, C)

                    # Resize detected_map and generated image to match gt_img
                    # print(gt_np.shape, detected_map_np.shape, generated_np.shape)
                    # Stack images horizontally
                    stacked_img = np.hstack([gt_np, detected_map_np, generated_np])

                    # Save the combined image
                    Image.fromarray(stacked_img).save(
                        f"picture-gene/batch_{batch_pos}_test_{index}_{img_name[index]}_{j+1}_stacked.png"
                    )
